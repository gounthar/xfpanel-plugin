...
public class XFPanelView extends ListView {
    ...
    protected transient Map<hudson.model.Queue.Item, Integer> placeInQueue =
            new HashMap<hudson.model.Queue.Item, Integer>();
    ...
    public XFPanelColors getColors() {
        XFPanelColors obj = new XFPanelColors(
                successfulBuildColor,
                buildFontColor,
                unstableBuildColor,
                buildFontColor,
                brokenBuildColor,
    ...
    public Collection<Job<?, ?>> getPrioritySortedJobs(Collection<Job<?, ?>> jobs, final boolean isConfiguration) {
        if (jobs != null) {
            ...
            final List<TopLevelItem> allItems =
                    getItems(); // this is expensive function, and used in 'contains', so collect it once to speed up
        ...
        for (hudson.model.Item i : Hudson.getInstance().getItems()) {
            ...
            if (itemName != null) {
                ...
                throw new FormException(
                        "Couldn't read jobs from the config file. Generate a new config-file: jenkins_job_url/configure",
                        "");
            }
        }
    ...
    public Pattern getJobNameReplaceRegExpPattern() {
        if (jobNameReplaceRegExpPattern == null
                && getJobNameReplaceRegExp() != null
                && getJobNameReplaceRegExp().length() > 0) {
    ...
    public static final class XFPanelViewDescriptor extends ViewDescriptor {
         ...
         * @param value represents the number of columns specified by the user.
    ...
    public static final class XFPanelColors {
        ...
        public static final XFPanelColors DEFAULT = new XFPanelColors(
                "#7E7EFF", "#FFFFFF", "#FFC130", "#FFFFFF", "#FF0000", "#FFFFFF", "#CCCCCC", "#FFFFFF");
